...
Джоба, генерирующая кэш на CI, что гоняется при мерже в мастер, теперь будет отгружать архив не только с контент контентом директории `$GRADLE_USER_HOME/caches/build-cache-1`, но и файлик `cache-keys.txt`. Джобы запускаемые в МРах, ориентируясь на этот файл будут чистить свой кэш от личшних ключей, а то что останется будут сохранять для дальнейших прогонов:

TODO: Тут диаграмма перекидывания файлов туда сюда между пайпами
...
Представленное здесь решение можно считать одним из самых эффективных способов сократить билд-тайм на CI, используя только бесплатные инструменты и не погружаясь в impact-анализ. В общем, топ за свои деньги (бесплатно епта).