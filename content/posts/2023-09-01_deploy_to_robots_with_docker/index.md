...
BuildKit использует Docker контейнер с эмулятором чтобы билдить ваши Docker образы. Эмулятору недоступна ваша локальная сеть, по адресу `http://localhost` он будет получать доступ ко внутренней сети Docker. Чтобы обойти эту проблему, в Docker есть специальный алиас `http://host.docker.internal`, ведущий в нашу обычную локальную сеть. Про алиас и конкретно наш юзкейс можно почитать подробнее в [документации Docker](https://docs.docker.com/desktop/networking/#i-want-to-connect-from-a-container-to-a-service-on-the-host).
